                ...
                <div key={field.key} className="space-y-1">
                  ...
                  {field.type === "select" && (
                    <select
                      value={String(config[field.key] ?? "")}
                      onChange={(e) => handleConfigChange(field.key, e.target.value)}
                      className="w-full px-2 py-1.5 pr-8 rounded-md border border-border bg-background text-foreground text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 shadow-sm transition-all appearance-none cursor-pointer"
                      style={{
